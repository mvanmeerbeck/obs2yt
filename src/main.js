...
import { app, globalShortcut, Menu, Tray } from 'electron/main';
import ffmpegPath from 'ffmpeg-static';
import WebSocket from 'ws';
import icon from './assets/icon.png';
import { extractFirstFrame } from './ffmpeg.js';
import { editThumbnail, loadTokens, uploadToYouTube } from './upload.js';

dotenv.config();

const OBS_WEBSOCKET_URL =
  process.env.OBS_WEBSOCKET_URL || 'ws://localhost:4455';
const NETWORK_PATH =
  process.env.NETWORK_PATH || '\\macbook-pro.local\Macintosh HD';
...
async function autoUploadToYouTube(videoPath) {
  try {
    ...
    console.log(
      `🎬 Video uploaded: https://www.youtube.com/watch?v=${result.id}`
    );
