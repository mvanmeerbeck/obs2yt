...
async function editThumbnail(videoId, thumbnailPath) {
  try {
    ...
    const newThumbnailPath = path.join(originalDir, 'thumbnails', `${videoId}${originalExt}`)
...
async function uploadToYouTube(videoPath, title, description, deleteAfterUpload = false) {
  try {
...
async function testUpload() {
  try {
    ...
    if (!testVideoPath) {
      console.log('⚠️ No test video found. Place a test.mp4 file in Videos, Desktop, or Documents folder.');
      console.log('Or specify a custom path:');
      console.log('const { uploadToYouTube, loadTokens } = require("./upload.js");');
      console.log('loadTokens();');
      console.log('uploadToYouTube("path/to/your/video.mp4", "Test Title", "Test Description");');
      return;
    }
...
if (import.meta.url === `file://${process.argv[1]}`) {
  ...
  async function main() {
    ...
    if (loadTokens()) {
      ...
      rl.question('\nDo you want to test upload with a video file? (y/n): ', async (answer) => {
        if (answer.toLowerCase() === 'y' || answer.toLowerCase() === 'yes') {
          await testUpload();
        } else {
          console.log('Ready to upload! Use the uploadToYouTube() function in your code.');
        }
        rl.close();
      });
    } else {
